...
void pin_set(GPIO_TypeDef * port_abc, uint16_t port_num, int number) {
	HAL_GPIO_WritePin(port_abc, port_num, GPIO_PIN_SET);
	HAL_Delay(number);
}

void pin_reset(GPIO_TypeDef * port_abc, uint16_t port_num, int number) {
	HAL_GPIO_WritePin(port_abc, port_num, GPIO_PIN_RESET);
	HAL_Delay(number);
}
...
int main(void) {
	...
	while (1) {
		...
		//Blinking RBG LED (reverse logic)

		pin_reset(GPIOA, GPIO_PIN_0, 100);
		pin_set(GPIOA, GPIO_PIN_0, 100); //custom function, work in progress
