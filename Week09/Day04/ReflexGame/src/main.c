...
int main(void) {
	...
	while (1) {
		...
		case GAME_INIT:
			...
			printf("Round %d \r\nPush any button if you are ready\r\n", round_number);
			...
			float random_time = (random_number % 10 + 1);
			...
			if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0)) {
				HAL_GPIO_WritePin(GPIOF, GPIO_PIN_8, GPIO_PIN_RESET);
				uint32_t end_count = HAL_GetTick();
				//float reaction = (end_count - start_count);
				printf("Your reaction time is ... Get ready for the next round!\r\n");
				round_number++;
			}
		}
